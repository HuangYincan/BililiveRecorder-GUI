...
- **跨平台证据以固定 head 的隔离 runner 结果为准。** 守护进程使用自己创建的 `Child`、管道和平台约束；本机单元测试或交叉类型检查不等于已安装产物验证，也不证明后端进程树或录制文件完整。共享主机上不运行终止类测试。
- Windows 守护进程创建自己的隐藏控制台，并将后端放入新控制组，先向该组发送 `CTRL_BREAK_EVENT` 触发 CLI 的取消/关停处理，再进行有界等待；不会附加到已有控制台，也不向组 0 广播。保留继承的 stdin/stdout/stderr，避免破坏 keepalive。控制台或 Job Object 建立失败时不启动后端；超时仍保留强制终止兜底。此路径的真实行为须由对应 CI 产物用例证明，不以关停日志替代录制文件完整性。
...
- **macOS 卡死场景的技术取舍（需决策，不是遗漏）**：macOS 既无 `PR_SET_PDEATHSIG` 也无作业对象。守护进程被强杀时后端会被 `launchd` 收养，应用既无法证明归属也不能安全发信号，只能点名其 PID 供人处理。守护进程**卡死**时应用只有两个选择，而安全的只有一个：
  ...
  代价是：macOS 上守护进程卡死时，后端可能继续运行，直到守护进程恢复——应用会明确告知，而不是假装已经收敛。它**不会**去拿后端的 PID 强行回收：一次可能打到无关进程的强制回收，不值得用来换取「看起来关干净了」。

  **界面告知的确切边界**（由维护者按「限制须准确说明、且不得提示按 PID 盲目终止」的要求确定）：

  - 提示文案 `UNRECLAIMED_BACKEND_MESSAGE` **不接收任何参数**，因此从结构上就无法写出进程号；守护进程上报的 PID 只写进 stderr，作为日志证据而不是操作指令。
  - 文案只说后端**可能**仍在运行、应用无法确认它已停止，**不宣称所有异常路径都能自动回收**——macOS 这一条就明确不能。
  - 文案明确**劝阻**按进程号手动终止，因为启动时从管道读到的 PID 此刻可能已指向别的进程；自动化规避了这个错误，再在弹窗里建议它就说不过去了。

  要真正堵上需要内核级回收器（launchd agent 或系统扩展）来收养孤儿，其签名与公证成本与这个缺陷不相称，因此**已知限制**为本阶段结论，不扩展系统级组件。
- **平台代码的验证程度**：`platform.rs` 可通过独立最小 crate 做 macOS/Linux/Windows 类型检查；原生 API 行为需实际 runner 验证。`artifact_lifecycle.rs` 的真实产物用例默认 ignored，只能在可丢弃 runner/VM 显式启用。正常退出、强杀、日志和文件完整性是独立判据。
- 安装包和退出证据逐个目标、逐个 SHA 记录；任何未验证平台或失败步骤都不能因其他 job 通过而视为通过。
