...
基于 Tauri 的跨平台 BililiveRecorder 桌面端。业务界面完全复用上游 CLI 内嵌的官方 WebUI，不复制、不修改、也不维护第二套录播界面。Tauri 只负责选择工作目录、分发和启动官方 CLI、健康检查、打开本地 WebUI 窗口、生命周期管理与桌面自动更新。
...
4. 启动时为后端分配随机本机端口，并执行：

   ```text
   BililiveRecorder.Cli run --bind http://127.0.0.1:<port> <工作目录>
   ```

5. `/api/version` 健康检查成功后，应用用独立桌面窗口打开 CLI 内置 WebUI。
6. macOS/Linux 停止时向 sidecar 发送 `SIGINT`，让录播姬保存配置并停止录制；超时后才强制终止。Windows 当前使用强制终止兜底，详见“已知限制”。

## 上游同步与桌面更新

1. `.github/workflows/sync-upstream.yml` 每天查询 BililiveRecorder 最新官方 Release。
2. 发现新版本后，自动更新 `upstream.json`、npm/Tauri/Cargo 版本并创建同步 PR；脚本同时校验各平台官方 CLI 资产齐全。
3. 同步 PR 合并到 `main` 后，`.github/workflows/release.yml` 在 macOS ARM64、macOS x64、Windows x64、Linux x64 上重新下载对应官方 CLI 并构建桌面安装包。
4. 发布流程使用 Tauri updater 私钥签名更新包，并生成 GitHub Release 的 `latest.json`。
5. 已安装应用启动时检查 `latest.json`；发现新版本后显示“下载并安装”，验证签名后原位更新并重启。
