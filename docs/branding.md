...
The visible Tauri `productName`, window/dialog title, app bundle/installers and auto-generated shortcuts use **Mikufans录播姬**. The brand implementation retained the existing identifiers for previously installed 2.20.1 clients; a later, separately reviewed desktop-shell version is required for publication:
...
## ASCII distribution identity after the stopped 2.20.2 attempt

The stopped `app-v2.20.2` release is an unpublished draft and its tag points to the reviewed build SHA. **Do not delete, replace, expose or reuse it.** Its [actual recorded 17 asset names](../scripts/fixtures/failed-app-v2.20.2-asset-names.json) show that Tauri CLI produced bundles named `Mikufans录播姬…`, and `tauri-action@v0` sent those Unicode names to GitHub, but the draft stored names such as `Mikufans._aarch64.app.tar.gz`. The precise updater-name gate rejected those files before publishing, leaving public latest at `app-v2.20.1`. This is an asset-upload identity mismatch; the installed app, icon, visible productName and updater signing public key did not change.

For the strictly higher shell version, distribution filenames are ASCII with `MikufansRecorder_[version]_[platform]_[arch][setup][ext]`, rendered by the pinned Tauri action. This is only an **upload name**: the `.app` filename, Info.plist, installer display text, desktop entry and white-background icon remain `Mikufans录播姬`. `scripts/release-asset-names.mjs` is the exact allow-list for all 17 uploaded files, including four updater payloads and their corresponding `.sig`. The publish job queries the **actual GitHub draft** after all four platform builds and refuses any missing, extra, colliding, un-digested, wrongly tagged or incorrectly targeted file **before creating a tag**. The manifest and final validator independently require the same exact updater/installer names and equality of each manifest signature with its uploaded `.sig` file. Recorded failed-draft names are a test fixture, not a substitute for this live upload check; successful structural linkage is not by itself a cryptographic re-verification of every downloaded installer.
